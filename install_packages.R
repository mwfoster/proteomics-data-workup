...
if (!package_ok("HarmonizR")) {
  tryCatch(
    {
      message("Installing optional Bioconductor package: HarmonizR")
      if (!package_ok("BiocManager")) {
        install.packages("BiocManager", repos = getOption("repos"), dependencies = c("Depends", "Imports", "LinkingTo"))
      }
      BiocManager::install("HarmonizR", ask = FALSE, update = FALSE)
    },
    error = function(e) {
      warning(
        "Optional package 'HarmonizR' could not be installed. The app will still run, ",
        "but the Batch correction tab will require HarmonizR. Install manually with ",
        "BiocManager::install('HarmonizR'). Details: ",
        conditionMessage(e),
        call. = FALSE
      )
    }
